...
@section('content')
    ...
    <div class="card shadow mb-4">
        ...
        <div class="card-body">
            <form action="{{ route('wishlist.add', ['wishlist' => $wishlist->id]) }}" method="post">
                @csrf
                @method('PUT')
                <div class="form-group">
                    <label class="text-primary">Tambah Dana</label>
                    <input type="hidden" name="amount" class="form-control" required placeholder="10000">
                    <input type="text" id="amount" class="form-control" value="{{ $wishlist->amount }}" required placeholder="10000">
                </div>
