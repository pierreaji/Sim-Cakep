...
@section('content')
    ...
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        ...
        <a href="{{ route('income.index') }}" class="d-inline-block btn btn-sm btn-primary shadow-sm"><i
        ...
        <div class="card-body">
            <form action="{{ route('income.update', ['income' => $income->id]) }}" method="post">
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="amount" value="{{ $income->amount }}" class="form-control" required
                        placeholder="10000">
                </div>
                <div class="form-group">
                    ...
                    <select name="id_category" id="category" class="form-control" required>
                        ...
                        @foreach ($category as $row)
                            <option value="{{ $row->id }}" {{ $row->id == $income->id_category ? 'selected' : '' }}>
                                {{ $row->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group">
                    ...
                    <input type="text" name="desc" value="{{ $income->desc }}" class="form-control" placeholder="Deskripsi">
